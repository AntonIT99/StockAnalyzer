...
class StockApp:
    ...
    def calculate_signal_summary(
        ...
        score_changes = StockApp.calculate_signal_score_changes(
            daily_data,
            daily_summary,
    ...
    def calculate_signal_score_changes(
        ...
        if daily_summary_as_of is not None:
            as_of = StockApp.align_timestamp_to_index(daily_summary_as_of, comparison_data.index)
        ...
        previous_summary = (
            StockApp.calculate_daily_structural_summary(daily_data, as_of=current_bar_date)
        ...
        if period_start_as_of is not None:
            period_start = StockApp.align_timestamp_to_index(
            ...
            if len(start_candidates) > 0:
                period_start_summary = StockApp.calculate_daily_structural_summary(
                    daily_data,
                    as_of=start_candidates[0] + pd.Timedelta(days=1)
                )

        return {
            "score_changes_last_day": StockApp.build_score_change_rows(
                current_summary,
                previous_summary
            ),
            "score_changes_period_start": StockApp.build_score_change_rows(
                current_summary,
